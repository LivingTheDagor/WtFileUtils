...
class BitStream(io.RawIOBase):
    ...
    def read_string(self, null_terminated=True, desc="_"):
        """
        reads a string from the bitstream neatly,
        :param null_terminated: if true, tries to read a null terminated string, if false, reads a pascal string
        """
        if null_terminated:
            does_hex = self.do_im_hex
            self.do_im_hex = False
            start_addr = self.current_bit_index
            stringbuff = bytearray()
            char = self.fetch(8)[0]
            while char != 0x00:
                stringbuff.append(char)
                char = self.fetch(8)[0]
            end_addr = self.current_bit_index
            if does_hex:
                bit_count = end_addr - start_addr
                add = 0 if self.current_bit_index % 8 == 0 or bit_count == 0 else 1
                self.write_stream.write(
                    f"u8 _{desc}_null_string_{self.current_bit_index}[{math.ceil(bit_count / 8) + add}] @ {self.current_bit_index // 8};")
                self.do_im_hex = True
            return stringbuff.decode("utf-8")
        else:
            return self.fetch(self.decode_uleb128(f"{desc}_pascal_header")*8, f"{desc}_pascal_string").decode("utf-8")


    def set_do_imhex(self, val: bool):
        self.do_im_hex = val == True # prevent bad data from being passed

    def set_ws(self, ws: WriteStream):
        self.ws = ws